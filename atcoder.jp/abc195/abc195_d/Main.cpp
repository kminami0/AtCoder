...
int main()
{
    ll N, M, Q;
    cin >> N >> M >> Q;
    vector<pair<ll, ll>> nimotsu(N);
    rep(i, N){
        cin >> nimotsu.at(i).second >> nimotsu.at(i).first;
    }
    sort(rbegin(nimotsu), rend(nimotsu));
    vector<ll> X(M);
    rep(i, M){
        cin >> X.at(i);
    }
